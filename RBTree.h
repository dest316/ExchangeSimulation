...
struct Node {
	...
	int count; // кол-во таких элементов, присутствующих в дереве
};
...
private:
	...
	void countOrder(Node<T>* node, int& value, int& count) {	//Изменённый метод preorder, который суммирует значения листьев и считает их количество
		if (node != TNULL)
		{
			value += node->data;
			count++;
	...
	double averageKeyValue(Node<T>* node) { //Здесь считаем среднее арифметическое
		int value = 0;
		int count = 0;
...
public:
	...
	void operator ++()
	{
		...
		{
			while (data.parent != obj.getRoot() && data.parent->data < data.data ) //в итераторе надо переименовать поле data, ибо это нечитаемо
			{
				data = data.parent;
			}
		}
